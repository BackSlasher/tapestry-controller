...
class QRPositionData(NamedTuple):
    ...
    ]  # (min_x, min_y, max_x, max_y) of QR code
...
def calculate_physical_positions_from_qr(
    ...
    for data in position_data:
        ...
        if screen_corner_positions and len(screen_corner_positions) >= 4:
            ...
            print(
                f"  Screen box: ({min_x:.0f},{min_y:.0f}) to ({max_x:.0f},{max_y:.0f}), size: {screen_width_px:.0f}x{screen_height_px:.0f}px"
            )
        else:
    ...
    print(
        f"Average screen size: {avg_screen_size:.0f}px → {typical_screen_size_mm}mm, scale: {scale_factor:.4f} mm/px"
    )
    ...
    for screen in screen_data:
        ...
        print(
            f"  FINAL {screen['hostname']}: ({layout_x:.0f}, {layout_y:.0f}) - from detected screen box"
        )
...
def generate_updated_config(
    original_config: Config, positions: Dict[str, Dict]
) -> Dict:
    ...
    for hostname, pos_data in positions.items():
        device_config = {
            ...
            "coordinates": {"x": int(pos_data["x"]), "y": int(pos_data["y"])},
