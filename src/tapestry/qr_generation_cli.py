...
def parse_args():
    ...
    parser.add_argument(
        "--output", "-o", help="Output file path (default: qr-{ip}.png)"
    )
...
def main():
    ...
    # Generate QR code
    print(
        f"Generating QR code for {args.ip} ({args.screen_type}) at {args.width}x{args.height}..."
    )
    qr_image = generate_positioning_qr_image(
        args.ip, args.screen_type, args.width, args.height
    )
